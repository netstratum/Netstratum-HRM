...
class HrmLeave extends CActiveRecord
{
	...
	public function attributeLabels()
	{
        ...
        public function getleaveid()
        {
            ...
            $session->open();
            $leavearr=Yii::app()->db->createCommand("SELECT a.name,b.id FROM hrm_leave_types a INNER JOIN hrm_employee_leave b ON a.id=b.leave_type WHERE a.active='Y' AND a.emp_appliable='Y' AND b.active='Y' AND b.emp_number='{$session['empnumber']}' and b.year = '".date('Y')."' ")->queryAll();    
            return $leavearr;        
        }
        ...
        public function getMybalanceLeave($leave_type)
        {
        	...
        	left join hrm_leave b on a.id = b.emp_leave_id and a.emp_number='{$session['empnumber']}' and b.approval not in ('cancel','reject') where a.year = ".date('Y')." and a.id = '$leave_type'  group by a.leave_type")->queryRow();    
            return $leavearr;        
...
//and b.emp_number='{$session['empnumber']}'
            $leavearr=Yii::app()->db->createCommand("select concat(d.emp_firstname,' ',d.emp_lastname) as username,b.id,a.name,b.leave_number,if (c.leave_days>0,(b.leave_number-sum(c.leave_days)),b.leave_number) as leave_balance from hrm_leave_types a 
                ...
                where a.emp_appliable = 'Y' and b.year = '".date('Y')."' $append  group by b.leave_type   $orderby limit $limit,$display_length")->queryAll();
        ...
        public function AllLeavebalance_cnt($emp_num,$leave_year)
        {
            ...
            if ($leave_year!='')
             ...
             $leavearr=Yii::app()->db->createCommand("select a.id,if (c.leave_days>0,(b.leave_number-sum(c.leave_days)),b.leave_number) as leave_balance from hrm_leave_types a 
                ...
                where a.emp_appliable = 'Y' and b.year = '".date('Y')."'  $append    group by b.leave_type   ")->queryAll();
      ...
      public function checkExpiryDate($leavetype){
		
	 $session = new CHttpSession;
            $session->open();

	$leavearr=Yii::app()->db->createCommand("select a.custom_leave_type,a.expiry_date from hrm_leave_types a 
                inner join hrm_employee_leave b on a.id = b.leave_type                 
                where a.emp_appliable = 'Y' and b.year = '".date('Y')."' and b.id = '".$leavetype."'")->queryRow();	
           return $leavearr;		
       ...
       public function leavebalance($limit,$display_length,$search,$column,$dir)
        {
         ...
         if ($search!='')
            ...
            $leavearr=Yii::app()->db->createCommand("select b.id,a.name,b.leave_number,if (c.leave_days>0,(b.leave_number-sum(c.leave_days)),b.leave_number) as leave_balance from hrm_leave_types a 
                inner join hrm_employee_leave b on a.id = b.leave_type 
                left join hrm_leave c on b.id =  c.emp_leave_id and (c.approval != 'reject' and c.approval != 'cancel')
                where a.emp_appliable = 'Y' and b.year = '".date('Y')."' and b.emp_number='{$session['empnumber']}'  $append  group by b.leave_type   $orderby limit $limit,$display_length")->queryAll();
        ...
        public function leavebalance_cnt()
        {
            ...
            $session->open();
             ...
             $leavearr=Yii::app()->db->createCommand("select a.id,if (c.leave_days>0,(b.leave_number-sum(c.leave_days)),b.leave_number) as leave_balance from hrm_leave_types a 
                inner join hrm_employee_leave b on a.id = b.leave_type 
                left join hrm_leave c on c.emp_leave_id = b.id
                where a.emp_appliable = 'Y' and b.year = '".date('Y')."' and b.emp_number='{$session['empnumber']}'    group by b.leave_type   ")->queryAll();
            return count($leavearr);    
...
/*
            ...
            */
            ...
            /*
          echo   "select concat(e.emp_firstname,' ',e.emp_lastname) as created_name,c.approval,a.name,c.id as leave_id,c.start_date,c.end_date,c.start_type,c.end_type,
                c.leave_comments,c.leave_days,c.apply_date,d.start_day_time,d.end_day_time
                from hrm_leave_types a inner join hrm_employee_leave b on a.id = b.leave_type
                inner join hrm_leave c on b.id = c.emp_leave_id
                left join hrm_leave_time d on c.id = d.hrm_leave_id
                left join hrm_employee e on c.createdby = e.emp_number
             where  c.emp_number='{$session['empnumber']}' $search_append $append group by c.id $orderby limit $limit,$display_length";
*/
            return $leavearr;    
...
/*
        	...
        	*/
        	// }else{
                ...
                left join hrm_report_to k on k.emp_number = c.emp_number and k.user_type = 'supervisor' AND k.user_id = {$session['empnumber']}
               	left join hrm_leave_time d on c.id = d.hrm_leave_id
...
//            exit();
	...
	public function assignbalanceLeave($leave_type,$empno){
       ...
       public function getEmpLeaves($empid)
       {
       		...
       		inner join hrm_employee_leave T2 on T1.id = T2.leave_type Where T1.active = 'Y' and T2.emp_number = '$empid' and T2.year = '".date('Y')."' ")->queryAll();
       ...
       public function checkSATSUNHOL($start_date,$end_date,$empnumber)
       {
        ...
        public function CancelLeave($leaveid,$leave_comments)
        {
         ...
         $leave_comments = Yii::app()->db->quoteValue($leave_comments.' <br /> Leave Applied Reason: '.$leave_data['leave_comments']);
