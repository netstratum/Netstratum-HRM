...
class HrmUserMaster extends CActiveRecord
{
	...
	public function attributeLabels()
	{
        ...
        {   
            ...
            $addressdata = Yii::app()->db->createCommand("SELECT a.emp_firstname,a.emp_lastname,b.user_name,b.mobile_number,b.id FROM hrm_employee a INNER JOIN hrm_user_master b ON a.emp_number=b.emp_number WHERE a.emp_number NOT IN(1,81)$append ORDER BY a.emp_firstname ASC")->queryAll();
            return $addressdata;
        }
        public function searchaddressbook($term)
        {   
            $searchbook = Yii::app()->db->createCommand("SELECT concat(emp_firstname,' ',emp_lastname) as label,emp_number FROM hrm_employee WHERE (emp_firstname LIKE '%{$term}%' OR emp_lastname LIKE '%{$term}%')")->queryAll();
            return $searchbook;
        }
